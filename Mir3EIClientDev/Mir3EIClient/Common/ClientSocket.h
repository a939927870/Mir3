...
protected:
	int					m_nCurRecv;
	CHAR				m_szBuff[8192*2/*8192 * 4*/];				// For Receive

	char				m_szEncodeDefMsg[32];			// For Send
	char				m_szEncodeBody[4096/*8192*/];			
	char				m_szPacket[4096/*8192 * 2*/];
...
public:
	...
	BOOL		ConnectToServer(HWND hWnd, CHAR* szID, INT nPort, DWORD dwMessage);
...
//	VOID		SendToServer(CHAR *pszPacket);
	...
	VOID		OnQueryChar( char* pszUserID, INT nCertifyCode );
	VOID		OnNewChar( char *pszId, int nHair, int nJob, int nSex );
	VOID		OnDelChar( char *pszCharName );
	VOID		OnSelChar( char* pszUserID, char *pszCharName );
	...
	VOID		SendRunMsg(WORD wIdent, INT nX, INT nY, INT nDir);//发送行走消息包
	...
	void		SendEncryption(char *pszKey);//ydq
