...
BOOL CItem::DrawItem(INT nX, INT nY, BYTE bItemType)
{
	...
	if ( bDrawItem )
	{
		...
		if( bItemType == _ITEM_TYPE_INVENTORY )//鞫刻鑒좆
		{
		...
		case 6:
			{
				...
				if ( m_stItemInfo.stStdItem.bSource/*sSpecialPower*/ > 0 )
					sprintf(&szLineBuf[1][strlen(szLineBuf[1])], "강도%d ", m_stItemInfo.stStdItem.bSource/*sSpecialPower*/);
...
*/		default:
			{
				...
				case 43:		// 각종광석들.
					// 두번째줄.
					sprintf(&szLineBuf[1][strlen(szLineBuf[1])], " 路좆%d 넣씹%d ", m_stItemInfo.stStdItem.bWeight, m_stItemInfo.nDura/1000);//무게-路좆,순도-넣씹
					g_xMainWnd.PutsHan(NULL, nX, nY+nLineGap, RGB(250, 250, 250), RGB(0, 0, 0), szLineBuf[1]);
					break;
