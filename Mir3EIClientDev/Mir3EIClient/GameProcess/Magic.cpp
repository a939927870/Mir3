		                 ...
		                 SHORT shTargetTileX, SHORT shTargetTileY, CActor* pxOwner, INT nTarget)
{
	...
	if ( pstMagicSpr )
	{
		...
		if ( nTarget )
		{
			if ( nTarget == g_xGameProc.m_pMyHero->m_dwIdentity )
			{
				m_pxTarget = g_xGameProc.m_pMyHero;
			}
			else
			{
...
/*			if ( nWaveNum > 10010 )
				m_nDuplicateNum = g_xSound.PlayActorWav(m_shTileX, m_shTileY, g_xGameProc.m_pMyHero->m_wPosX, g_xGameProc.m_pMyHero->m_wPosY, nWaveNum, 100, bLoop);
*/		}
...
/*			if ( nWaveNum > 10010 )
				g_xSound.ChgPlayingSet(nWaveNum, m_nDuplicateNum, m_shTileX, m_shTileY, g_xGameProc.m_pMyHero->m_wPosX, g_xGameProc.m_pMyHero->m_wPosY);
*/		}
...
//		   if ( 마법이 통과할수 없으면 )
	...
	if ( bCrash )
	{
		...
		if ( timeGetTime() - dwLastTick > 100 )
		{
			...
			if ( nWaveNum > 10000 )
				g_xSound.PlayActorWav(m_shTileX, m_shTileY, g_xGameProc.m_pMyHero->m_wPosX, g_xGameProc.m_pMyHero->m_wPosY, nWaveNum);
		}
		dwLastTick = timeGetTime();
	}
