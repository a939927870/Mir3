...
CMirSound::~CMirSound()
{
	...
	if(m_ppBuffer!=NULL)
	{
		for(int j =0;j<MAX_KINDS_OF_WAVE;j++)			// Buffer Class
		{
			m_ppBuffer[j]->Release();
			delete m_ppBuffer[j];
		}
//fix me		delete[] m_ppBuffer;
	}
	...
	if(m_pBgmListHeader != NULL)	delete m_pBgmListHeader;
	if(m_pBgmFileList != NULL)	delete []m_pBgmFileList;
...
void CMirSound::ReadBgmFileList(char* fName)
{
	DWORD dwReadSize;
	...
	{
		//	Header Reading
		m_pBgmListHeader = new WAVELISTHEADER;
		ReadFile(hFile,m_pBgmListHeader,sizeof(WAVELISTHEADER),&dwReadSize,NULL);
	
		DWORD nLen = GetFileSize(hFile, NULL);
		m_pBgmFileList = new CHAR[nLen - sizeof(WAVELISTHEADER)];
		m_pBgmListHeader->FieldCount = nLen - sizeof(WAVELISTHEADER);

		ReadFile(hFile, m_pBgmFileList, nLen - sizeof(WAVELISTHEADER), &dwReadSize,NULL);
...
BOOL CMirSound::FindBGMFileName(CHAR*szMapName, CHAR* szMp3FileName)
{
	for(int i = 0; i < m_pBgmListHeader->FieldCount; i++ )
	{
		if( m_pBgmFileList[i] == '[' )
		{
			i++;
			if( strcmp(m_pBgmFileList+i, szMapName ) == 0 )
			{
				i += strlen(szMapName);
...
//꺄렴교쒼稜있
void CMirSound::PlayBkMusicMp3(BOOL Looped,  CHAR* szMapName)
{
	if(m_bBGMPlay)
	{
		...
		FindBGMFileName(szMapName,szMp3FileName);
