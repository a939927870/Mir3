...
VOID CCharacterProcess::Load()
{
	...
	m_Image.NewLoad(".\\Data\\interface1c.wil", TRUE);
	// 임시
	//
	m_ConnectionState = CONNECT_FAIL;

	g_xLoginSocket.ConnectToServer(g_xMainWnd.GetSafehWnd(), g_szServerIP, g_nServerPort, ID_SOCKCLIENT_EVENT_MSG);
...
VOID CCharacterProcess::SetNextProc()
{
	D3DMATRIX	matProj;
	DeleteProc();//
//YDQ 	g_xLoginSocket.m_pxDefProc = g_xMainWnd.m_pxDefProcess = &g_xGameProc;
//	g_xLoginSocket.m_pxDefProc = &g_xGameProc;//쉥무痰젯쌈맣냥槨g_xGameProc쉔접降唐젯쌈
	g_xMainWnd.m_pxDefProcess = &g_xTeamManage;//맣긴句口뇹잿변鑒
...
VOID CCharacterProcess::OnConnectToServer()
{
	...
	g_xLoginSocket.OnQueryChar( g_szUserID, g_nCertifyCode );
}
	...
	case	SM_STARTPLAY:			// Start Game
		{
			...
			g_xLoginSocket.DisconnectToServer();		// Char sever와의 연결 종료
		...
		case CLICK_YES:
			{
				...
				switch(m_nMessage)
				{
					case 5:
					{
						...
						g_xLoginSocket.OnDelChar( m_CList.GetCurrentData()->ChrName);
