				...
				{	
					...
					case ID_SEL_CHR_START_BTN:
					{
						...
						{	m_nBlend = 0;
							...
							g_xLoginSocket.OnSelChar(g_szUserID, m_pxSList->GetCurrentData()->ChrName);
...
VOID CSelectChr::Render(INT	nLoopTime)
{
	...
	if(m_fIsActive)
	{
		...
		for (i = ID_SEL_CHR_NEW_BTN; i <= ID_SEL_CHR_START_BTN ; i++)	// 단추 임시
		{
			...
			{
				m_pxImage->NewSetIndex(m_pBtn[i]->m_nButtonID+1);
			...
			if(m_pBtn[i]->m_nState==BUTTON_STATE_UP)		// Draw Button when Button state is Downed..
			{
